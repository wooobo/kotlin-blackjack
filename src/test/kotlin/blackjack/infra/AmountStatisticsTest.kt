...
class AmountStatisticsTest {
    ...
    @Test
    fun `blackjack 이 아닌 경우 배팅액을 받는다`() {
        val player = createPlayerWithNonBlackjack("player", 1000)
        amountStatisticsBuilder.onWin(player)

        val actual = amountStatisticsBuilder.build()

        assertAll(
            { assertThat(actual.playerProfits["player"]).isEqualTo(Money(1_000)) },
    ...
    private fun createPlayerWithBlackjack(name: String, betAmount: Int): Player {
        ...
        player.bet(Money(betAmount))
        player.receive(Deck(listOf(
